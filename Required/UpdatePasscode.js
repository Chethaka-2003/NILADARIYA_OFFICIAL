...
const UpdatePasscode = ({ navigation }) => {
  ...
  const handleSetPasscode = async () => {
    ...
    if (passcode === confirmPasscode) {
      ...
      setConfirmPasscode('');
      setErrorMessage('');
      navigation.goBack(); // Navigate back to the settings page after setting the passcode
    } else {
  ...
  const renderPasscodeDots = (code) => {
    return Array.from({ length: 6 }).map((_, i) => (
      <View
        ...
        style={[
          styles.passcodeDot,
          { backgroundColor: i < code.length ? '#195b8c' : 'transparent' },
        ]}
      />
    ));
  };
