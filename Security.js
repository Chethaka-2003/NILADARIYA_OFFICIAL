...
export default function ChangePassword() {
  ...
  const handleSendVerification = () => {
    ...
    axios.post('http://192.168.1.136:4000/auth/send-verification', { email, isPasswordReset: true })
      .then(res => {
  ...
  const handleSubmitVerification = () => {
    ...
    axios.post('http://192.168.1.136:4000/auth/verify-code', { email, code: verificationCode })
      .then(res => {
        if (res.data.status === "OK") {
  ...
  const handlePasswordChange = () => {
    ...
    axios.post('http://192.168.1.136:4000/auth/change-password', { email, newPassword, code: verificationCode })
      .then(res => {
        if (res.data.status === "OK") {
          setAlertTitle('Success');
